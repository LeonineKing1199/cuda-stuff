...
*/
template <typename T>
auto gen_cartesian_domain(int const grid_length) -> host_vector<point_t<T>>
{
  int const num_points = grid_length * grid_length * grid_length;
  host_vector<point_t<T>> domain;
...
template <typename T>
auto sort_by_peanokey(host_vector<point_t<T>>& domain) -> void
{
  host_vector<peanokey> keys{
    make_transform_iterator(domain.begin(), peanokey_hash<T>{}),
    make_transform_iterator(domain.end(), peanokey_hash<T>{})};
